...
OpenRouter's gateway has a built-in retry mechanism that handles transient network errors and provider timeouts automatically.
...
const response = await openai.chat.completions.create({
  ...
  // @ts-expect-error OpenRouter custom properties
  extra: {
